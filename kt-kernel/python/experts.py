...
class KTMoEWrapper:
    ...
    def __new__(
        ...
        # Optional exact checkpoint layer prefix (for example
        # ``model.layers.45`` for GLM-5-Next's appended MTP block).
        ...
        Args:
            ...
            weight_base_key: Exact native-checkpoint layer prefix when
                auto-detection is inappropriate, such as ``model.layers.45``
                for an appended GLM MTP stage.
...
def _create_inference_wrapper(
    ...
    if weight_base_key is not None:
        if backend_cls is not NativeMoEWrapper:
            raise ValueError(
                "weight_base_key is supported only by native SafeTensor "
