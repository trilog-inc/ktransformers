...
def _select_mxfp4_backend():
    ...
    if forced == "amx":
        ...
        if not _COMPILED_WITH_AMX_BF16:
            raise RuntimeError(
                "KT_MXFP4_BACKEND=amx requested, but this extension was built "
                "without AMX-BF16. Recompile with KTRANSFORMERS_CPU_USE_AMX=ON."
            )
        if not _HOST_HAS_AMX_BF16:
            raise RuntimeError(
                "KT_MXFP4_BACKEND=amx requested, but the host lacks "
    ...
    if forced == "avx2":
        if not _HAS_AVX2_MXFP4_SUPPORT:
            raise RuntimeError(
                "KT_MXFP4_BACKEND=avx2 requested, but "
                "AVX2MXFP4UE8M0_MOE is not compiled in. Recompile with "
                "AVX2 + FMA enabled."
            )
    ...
    if _HAS_MXFP4_SUPPORT and (
        not _COMPILED_WITH_AMX_BF16 or _HOST_HAS_AMX_BF16
    ):
        return AMXMXFP4UE8M0_KGroup_MOE
...
def _select_nvfp4_backend(group_size: int):
    ...
    """
    forced = os.getenv(
        "KT_NVFP4_BACKEND", os.getenv("KT_MXFP4_BACKEND", "amx")
    ).strip().lower()
    ...
    if forced == "amx":
        ...
        if AMXFP4_KGroup_MOE is None:
            raise RuntimeError(
                "KT_NVFP4_BACKEND=amx requested, but AMXFP4_KGroup_MOE is not "
                "compiled in. Recompile with AVX512F + AVX512BW + "
                "AVX512_BF16 enabled."
            )
    ...
    def load_weights(self, physical_to_logical_map_cpu: torch.Tensor):
        ...
        if self.method == "BF16":
            ...
            if self.method == "RAWINT4":
                ...
                assert first_gate_scale.dtype == torch.uint8, (
                    "Expected native uint8 UE8M0 scales for MXFP4"
                )
        ...
        if self.method == "NVFP4":
            gate_raw_scale_ptrs = [
                [0 if t is None else t.data_ptr() for t in self.gate_raw_scales]
            ]
            up_raw_scale_ptrs = [
                [0 if t is None else t.data_ptr() for t in self.up_raw_scales]
            ]
            down_raw_scale_ptrs = [
                [0 if t is None else t.data_ptr() for t in self.down_raw_scales]
            ]
        else:
            gate_raw_scale_ptrs = []
            up_raw_scale_ptrs = []
