...
inline bool enable_amx() {
  // XCOMP permission is a per-thread property. CPUInfer workers are persistent,
  // so avoid two arch_prctl syscalls before every AMX task after the first
  // successful permission request on that worker.
