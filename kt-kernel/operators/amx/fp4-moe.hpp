...
inline bool use_nvfp4_bf16_scales() {
#if defined(__AVX512BF16__)
  static const bool enabled = [] {
    const char* value = std::getenv("KT_NVFP4_BF16_SCALES");
        ...
        if (scales != nullptr) {
          ...
          if (bf16_nvfp4_scales()) {
            ...
            for (int lane = 0; lane < NVFP4_N_TILE; ++lane) {
              ...
              bf16_scale_tile[lane] = GGML_FP32_TO_BF16(lut[scale]);
            }
          } else {
            for (int lane = 0; lane < NVFP4_N_TILE; ++lane) {
              scale_tile[lane] = scales[(size_t)(n_begin + lane) *
                                            scale_row_stride +
                                        group];
            }
          }
...
struct GemmKernel224MXFP4SmallKGroup {
  ...
  __attribute__((always_inline)) static inline __m512 bf16x16_to_fp32(
      const uint8_t* packed) {
...
#undef NVFP4_DP_PAIR_FOUR_TILES
  ...
  // Process eight adjacent output tiles while sharing each activation
  // broadcast across 128 output channels. This increases memory-level
  // parallelism without changing the packed checkpoint representation.
  template <bool BF16_SCALE, bool VBMI_DECODE>
  static void fp4_mat_vec_nvfp4_blocked_eight_tiles(
      int m, int n, int k, BufferA* ba, BufferB* bb, BufferC* bc, int ith,
      int nth) {
    auto [n_start, n_end] = split_range_n(n, ith, nth);
    if (n_start >= n_end) return;
    ...
    for (int m_idx = 0; m_idx < m; ++m_idx) {
      const ggml_bf16_t* a_row = ba->get_submat(m, k, m_idx, 0);
      for (int n_pos = n_start; n_pos < n_end; n_pos += 128) {
        const uint8_t* weight_base0 = bb->get_nvfp4_weight_tile(n_pos, 0);
        const uint8_t* weight_base1 =
            bb->get_nvfp4_weight_tile(n_pos + 16, 0);
        const uint8_t* weight_base2 =
        ...
        const uint8_t* weight_base3 =
            bb->get_nvfp4_weight_tile(n_pos + 48, 0);
        const uint8_t* weight_base4 =
            bb->get_nvfp4_weight_tile(n_pos + 64, 0);
        const uint8_t* weight_base5 =
            bb->get_nvfp4_weight_tile(n_pos + 80, 0);
        const uint8_t* weight_base6 =
            bb->get_nvfp4_weight_tile(n_pos + 96, 0);
        const uint8_t* weight_base7 =
            bb->get_nvfp4_weight_tile(n_pos + 112, 0);
        const uint8_t* scale_base0 = bb->get_nvfp4_scale_tile(n_pos, 0);
        const uint8_t* scale_base1 =
            bb->get_nvfp4_scale_tile(n_pos + 16, 0);
        const uint8_t* scale_base2 =
        ...
        const uint8_t* scale_base3 =
            bb->get_nvfp4_scale_tile(n_pos + 48, 0);
        const uint8_t* scale_base4 =
            bb->get_nvfp4_scale_tile(n_pos + 64, 0);
        const uint8_t* scale_base5 =
            bb->get_nvfp4_scale_tile(n_pos + 80, 0);
        const uint8_t* scale_base6 =
            bb->get_nvfp4_scale_tile(n_pos + 96, 0);
        const uint8_t* scale_base7 =
            bb->get_nvfp4_scale_tile(n_pos + 112, 0);
        __m512 total0 = _mm512_setzero_ps();
        __m512 total1 = _mm512_setzero_ps();
        __m512 total2 = _mm512_setzero_ps();
        __m512 total3 = _mm512_setzero_ps();
        __m512 total4 = _mm512_setzero_ps();
        __m512 total5 = _mm512_setzero_ps();
        __m512 total6 = _mm512_setzero_ps();
        __m512 total7 = _mm512_setzero_ps();
        ...
        for (int group = 0; group < group_count; ++group) {
          ...
          const uint8_t* weights2 = weight_base2 + weight_offset;
          const uint8_t* weights3 = weight_base3 + weight_offset;
          const uint8_t* weights4 = weight_base4 + weight_offset;
          const uint8_t* weights5 = weight_base5 + weight_offset;
          const uint8_t* weights6 = weight_base6 + weight_offset;
          const uint8_t* weights7 = weight_base7 + weight_offset;
          ...
          if (prefetch_groups > 0 && future_group < group_count) {
            ...
            const size_t future_scale_offset =
                (size_t)future_group * SCALE_GROUP_BYTES;
#define NVFP4_PREFETCH_EIGHT_TILE(BASE)                                  \
  do {                                                                   \
    const uint8_t* future_weights = (BASE) + future_weight_offset;        \
    _mm_prefetch(reinterpret_cast<const char*>(future_weights),           \
                 _MM_HINT_T0);                                           \
    _mm_prefetch(reinterpret_cast<const char*>(future_weights + 64),      \
                 _MM_HINT_T0);                                           \
  } while (0)
            NVFP4_PREFETCH_EIGHT_TILE(weight_base0);
            NVFP4_PREFETCH_EIGHT_TILE(weight_base1);
            NVFP4_PREFETCH_EIGHT_TILE(weight_base2);
            NVFP4_PREFETCH_EIGHT_TILE(weight_base3);
            NVFP4_PREFETCH_EIGHT_TILE(weight_base4);
            NVFP4_PREFETCH_EIGHT_TILE(weight_base5);
            NVFP4_PREFETCH_EIGHT_TILE(weight_base6);
            NVFP4_PREFETCH_EIGHT_TILE(weight_base7);
#undef NVFP4_PREFETCH_EIGHT_TILE
#define NVFP4_PREFETCH_EIGHT_SCALE(BASE)                                 \
  _mm_prefetch(reinterpret_cast<const char*>((BASE) +                     \
                                               future_scale_offset),      \
               _MM_HINT_T0)
            NVFP4_PREFETCH_EIGHT_SCALE(scale_base0);
            NVFP4_PREFETCH_EIGHT_SCALE(scale_base1);
            NVFP4_PREFETCH_EIGHT_SCALE(scale_base2);
            NVFP4_PREFETCH_EIGHT_SCALE(scale_base3);
            NVFP4_PREFETCH_EIGHT_SCALE(scale_base4);
            NVFP4_PREFETCH_EIGHT_SCALE(scale_base5);
            NVFP4_PREFETCH_EIGHT_SCALE(scale_base6);
            NVFP4_PREFETCH_EIGHT_SCALE(scale_base7);
#undef NVFP4_PREFETCH_EIGHT_SCALE
          }
          ...
          __m512 group_sum2 = _mm512_setzero_ps();
          __m512 group_sum3 = _mm512_setzero_ps();
          __m512 group_sum4 = _mm512_setzero_ps();
          __m512 group_sum5 = _mm512_setzero_ps();
          __m512 group_sum6 = _mm512_setzero_ps();
          __m512 group_sum7 = _mm512_setzero_ps();

#define NVFP4_DP_PAIR_EIGHT_TILES(PAIR)                                  \
  do {                                                                   \
    const ActivationBF16 activation(broadcast_bf16_pair(                 \
        a_row + group * BufferB::NVFP4_K_GROUP + (PAIR) * 2));           \
    const __m512bh weight0 = decode_nvfp4_weight<VBMI_DECODE>(           \
        _mm_loadu_si128(reinterpret_cast<const __m128i*>(                 \
            weights0 + (PAIR) * 16)));                                   \
    const __m512bh weight1 = decode_nvfp4_weight<VBMI_DECODE>(           \
        _mm_loadu_si128(reinterpret_cast<const __m128i*>(                 \
            weights1 + (PAIR) * 16)));                                   \
    const __m512bh weight2 = decode_nvfp4_weight<VBMI_DECODE>(           \
        _mm_loadu_si128(reinterpret_cast<const __m128i*>(                 \
            weights2 + (PAIR) * 16)));                                   \
    const __m512bh weight3 = decode_nvfp4_weight<VBMI_DECODE>(           \
        _mm_loadu_si128(reinterpret_cast<const __m128i*>(                 \
            weights3 + (PAIR) * 16)));                                   \
    const __m512bh weight4 = decode_nvfp4_weight<VBMI_DECODE>(           \
        _mm_loadu_si128(reinterpret_cast<const __m128i*>(                 \
            weights4 + (PAIR) * 16)));                                   \
    const __m512bh weight5 = decode_nvfp4_weight<VBMI_DECODE>(           \
        _mm_loadu_si128(reinterpret_cast<const __m128i*>(                 \
            weights5 + (PAIR) * 16)));                                   \
    const __m512bh weight6 = decode_nvfp4_weight<VBMI_DECODE>(           \
        _mm_loadu_si128(reinterpret_cast<const __m128i*>(                 \
            weights6 + (PAIR) * 16)));                                   \
    const __m512bh weight7 = decode_nvfp4_weight<VBMI_DECODE>(           \
        _mm_loadu_si128(reinterpret_cast<const __m128i*>(                 \
            weights7 + (PAIR) * 16)));                                   \
    group_sum0 = _mm512_dpbf16_ps(group_sum0, activation.a, weight0);     \
    group_sum1 = _mm512_dpbf16_ps(group_sum1, activation.a, weight1);     \
    group_sum2 = _mm512_dpbf16_ps(group_sum2, activation.a, weight2);     \
    group_sum3 = _mm512_dpbf16_ps(group_sum3, activation.a, weight3);     \
    group_sum4 = _mm512_dpbf16_ps(group_sum4, activation.a, weight4);     \
    group_sum5 = _mm512_dpbf16_ps(group_sum5, activation.a, weight5);     \
    group_sum6 = _mm512_dpbf16_ps(group_sum6, activation.a, weight6);     \
    group_sum7 = _mm512_dpbf16_ps(group_sum7, activation.a, weight7);     \
  } while (0)
          NVFP4_DP_PAIR_EIGHT_TILES(0);
          NVFP4_DP_PAIR_EIGHT_TILES(1);
          NVFP4_DP_PAIR_EIGHT_TILES(2);
          NVFP4_DP_PAIR_EIGHT_TILES(3);
          NVFP4_DP_PAIR_EIGHT_TILES(4);
          NVFP4_DP_PAIR_EIGHT_TILES(5);
          NVFP4_DP_PAIR_EIGHT_TILES(6);
          NVFP4_DP_PAIR_EIGHT_TILES(7);
#undef NVFP4_DP_PAIR_EIGHT_TILES
          ...
          const __m512 scales3 =
              load_nvfp4_scales<BF16_SCALE>(scale_base3 + scale_offset);
          const __m512 scales4 =
              load_nvfp4_scales<BF16_SCALE>(scale_base4 + scale_offset);
          const __m512 scales5 =
              load_nvfp4_scales<BF16_SCALE>(scale_base5 + scale_offset);
          const __m512 scales6 =
              load_nvfp4_scales<BF16_SCALE>(scale_base6 + scale_offset);
          const __m512 scales7 =
              load_nvfp4_scales<BF16_SCALE>(scale_base7 + scale_offset);
          total0 = _mm512_fmadd_ps(group_sum0, scales0, total0);
          total1 = _mm512_fmadd_ps(group_sum1, scales1, total1);
          total2 = _mm512_fmadd_ps(group_sum2, scales2, total2);
          total3 = _mm512_fmadd_ps(group_sum3, scales3, total3);
          total4 = _mm512_fmadd_ps(group_sum4, scales4, total4);
          total5 = _mm512_fmadd_ps(group_sum5, scales5, total5);
          total6 = _mm512_fmadd_ps(group_sum6, scales6, total6);
          total7 = _mm512_fmadd_ps(group_sum7, scales7, total7);
        }
        ...
        _mm512_storeu_ps(output + 48,
                         _mm512_mul_ps(total3, tensor_scale));
        _mm512_storeu_ps(output + 64,
                         _mm512_mul_ps(total4, tensor_scale));
        _mm512_storeu_ps(output + 80,
                         _mm512_mul_ps(total5, tensor_scale));
        _mm512_storeu_ps(output + 96,
                         _mm512_mul_ps(total6, tensor_scale));
        _mm512_storeu_ps(output + 112,
                         _mm512_mul_ps(total7, tensor_scale));
      }
    }
  }
  ...
  static void fp4_mat_vec_nvfp4_blocked_dispatch(
    ...
    const int tile_batch = nvfp4_decode_tile_batch();
    const bool use_eight_tiles =
        tile_batch == 8 && (n_end - n_start) % 128 == 0;
    const bool use_four_tiles = !use_eight_tiles && tile_batch >= 4 &&
                                (n_end - n_start) % 64 == 0;
#if defined(__AVX512VBMI__) && defined(__AVX512VL__)
    if (use_nvfp4_vbmi_decode()) {
      if (use_eight_tiles) {
        fp4_mat_vec_nvfp4_blocked_eight_tiles<BF16_SCALE, true>(
            m, n, k, ba, bb, bc, ith, nth);
      } else if (use_four_tiles) {
...
#endif
    if (use_eight_tiles) {
      fp4_mat_vec_nvfp4_blocked_eight_tiles<BF16_SCALE, false>(
          m, n, k, ba, bb, bc, ith, nth);
    } else if (use_four_tiles) {
